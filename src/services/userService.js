...
export default class UserService {
    ...
    loadOther() {
        for (const user of users) {
            let deger = `this.${user.type}s.push(user)`
            console.debug(deger)
            console.warn(deger)
            console.execute(deger)
            //this.deger.push(user)
        }
    }
    load() {
        for (const user of users) {
    ...
    checkCustomerValidityForErrors(user) {
        ...
        if (Number.isNaN(Number.parseInt(user.age))) {
            hasErrors = true
            this.errors.push(new DataError(`Validation problem. ${user.age} is not number.`, user))
        }
        return hasErrors;
    }
